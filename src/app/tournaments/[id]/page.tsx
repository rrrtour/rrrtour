...
import { formatCountdown, isRegistrationOpen } from '@/lib/utils';
...
export default function PublicTournamentPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Only allow OPEN or LIVE for public view
      if (!t || (t.status !== 'OPEN' && t.status !== 'LIVE')) {
        setNotFound(true);
        setLoading(false);
        return;
      }
  ...
  const handleRegister = async () => {
    if (!profile) {
      router.push('/auth/login?role=athlete');
      return;
    }
    if (profile.role !== 'athlete') {
      setError('Only athletes can register for tournaments.');
      return;
    }
    if (!tournament) return;
    ...
    if (regError) {
      setError(
        regError.message.includes('duplicate') ||
          regError.message.includes('unique')
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="animate-pulse space-y-6">
          ...
          <div className="h-32 bg-surface-700 rounded-xl" />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-text-muted mb-8">
          This tournament doesn&apos;t exist, has been archived, or is not
  ...
  const time = new Date(tournament.date).toLocaleTimeString('en-US', {
    hour: 'numeric',
    minute: '2-digit',
  });
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              {tournament.is_private && (
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-500/15 text-purple-400 border border-purple-500/20">
                  Invite Only
                </span>
              )}
            </div>
          ...
          <div className="card p-6 space-y-4">
            ...
            <div className="flex items-start gap-3">
              <svg
                className="w-5 h-5 text-text-muted mt-0.5 shrink-0"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {tournament.location_map_url && (
                  <a
                    href={tournament.location_map_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-brand-red hover:text-brand-red-light text-sm"
                  >
                    View on Map →
                  </a>
                )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="font-medium">
                    {regCount} / {tournament.max_participants} registered
                  </p>
        ...
        <div className="lg:col-span-1">
          <div className="card p-6 sticky top-24 space-y-4">
            ...
            {/* LIVE state — registration closed */}
            {tournament.status === 'LIVE' && !registration && (
              <div className="space-y-3">
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 text-sm text-red-400 flex items-center gap-2">
                  ...
                  This tournament is live. Registration is closed.
                </div>
              </div>
            )}

            {/* OPEN state — registration available or closed by deadline/capacity */}
            {tournament.status === 'OPEN' && !regCheck.open && !registration && (
                ...
                <div className="space-y-4">
                  {isPaid && tournament.external_payment_url && (
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 text-sm text-amber-400">
                      This is a paid tournament. After registering, complete
                      payment and upload a screenshot.
                    </div>
                  )}
                  ...
                  {!profile ? (
                    ...
                    <p className="text-text-muted text-sm">
                      Only athletes can register for tournaments.
                    </p>
                  ) : (
                    <button
            ...
            {/* Not logged in and tournament is LIVE */}
            {!profile && tournament.status === 'LIVE' && (
              <Link
                href="/auth/login?role=athlete"
                className="btn-secondary w-full text-center block text-sm"
              >
